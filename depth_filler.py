...
if __name__ == "__main__":
	...
	imgname = "/home/minghanz/Repos/pcd_viewer/Thu Feb 20 23:48:15 2020/3_rgb_1.png"
	depname = "/home/minghanz/Repos/pcd_viewer/Fri Feb 21 14:05:23 2020/3_dep_gt_1.png"
	outname = "/home/minghanz/Repos/pcd_viewer/Fri Feb 21 14:05:23 2020/3_dep_gt_filled_4.png"
	rgb = Image.open(imgname)
	rgb_array = np.array(rgb).astype(np.float32)
	rgb_array = rgb_array/255
	print("rgb.shape:", rgb_array.shape)
	print("rgb.max, min:", rgb_array.max(), rgb_array.min())
	
	dep = Image.open(depname)
	dep_array = np.array(dep).astype(np.float32)
	print("dep.shape:", dep_array.shape)
	print("dep.max, min:", dep_array.max(), dep_array.min())

	output = fill_depth_colorization(imgRgb=rgb_array, imgDepthInput=dep_array, alpha=0.2)
	outimg = Image.fromarray(output.astype(np.uint8), mode="L")
	outimg.save(outname)